	...
	if ($action == 'add') {
		if (!empty($_SESSION['limit']['extensions']['numeric'])) {
			...
			if ($total_extensions >= $_SESSION['limit']['extensions']['numeric']) {
				...
				header('Location: vsa_ext.php'.(isset($page) && is_numeric($page) ? '?page='.$page : null));
				exit;
			}
		}
	}
		...
		//add or update the database
			if (empty($_POST["persistformvar"]) || $_POST["persistformvar"] != "true") {
				...
				//set the message and redirect
					...
					if ($range > 1) {
						header("Location: vsa_dial.php");
					}
					else {
						header("Location: vsa_dial.php?id=".$extension_uuid.(isset($page) && is_numeric($page) ? '&page='.$page : null));
					}
					exit;
			}
	}
